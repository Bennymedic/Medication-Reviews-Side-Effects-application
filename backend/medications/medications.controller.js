...
export const checkMedicationName = async (req, res, next) => {
    try {
        const {name} = req.body;
        const results = await Medication.findOne({name: name});
        res.json({ success: true, data: results ? {name: results.name} : null });
    } catch (error) {
        next(error);
    }
};
